...
void mme_state_operational(fsm_t *s, event_t *e)
{
    ...
    switch (event_get(e))
    {
        ...
        case MME_EVT_GTPV2_MESSAGE:
        {
            ...
            /* Check if the message type is UE_CONTEXT_TRANSFER */
            if (message.h.type == GTP_UE_CONTEXT_TRANSFER)
            {
                d_print_hex(pkbuf->payload, pkbuf->len);
                mme_s10_handle_ue_context_transfer(&message.ue_context_transfer);
                pkbuf_free(pkbuf);
                break;
            }
            ...
            switch(message.h.type)
            {
                ...
                case GTP_MODIFY_BEARER_RESPONSE_TYPE:
                    ...
                    if (mme_ue)
                    {
                        mme_s10_send_ue_context_transfer(mme_ue);
#if 1
                        mme_ue_print(mme_ue);
#endif
                    }
 ...
 */
        ...
        case MME_EVT_GTPV2_T3_HOLDING:
        case MME_EVT_GTPV2_T3_RESPONSE:
        {
            gtp_xact_timeout(event_get_param1(e), event_get(e));
            break;
        }
        case MME_EVT_REMOTE_MME_RESTORE:
        {
            ip_t addr;
            //void *arg1 = event_get_param1(e);
            mme_remote_context_t *mme_remote_context = NULL;

            /* Find mme_remote_context by ipaddr */
            mme_remote_context = mme_remote_context_find_by_ip(&addr);

            /* Restore UE under mme_remote_context */
            mme_remote_restore(mme_remote_context);
