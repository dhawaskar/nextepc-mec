...
index_declare(mme_remote_context_pool, mme_remote_context_t, 16);
index_declare(mme_remote_ue_pool, mme_remote_ue_t, MAX_POOL_OF_UE);
...
status_t mme_context_init()
{
    ...
    list_init(&self.remote_mme_list); /* Initialize remote mme list */
    list_init(&self.mme_remote_context_list); /* Initialize remote mme list */
    ...
    index_init(&mme_remote_context_pool, 16);
    index_init(&mme_remote_ue_pool, MAX_POOL_OF_UE);
...
status_t mme_context_final()
{
    ...
    index_final(&mme_remote_context_pool);
    index_final(&mme_remote_ue_pool);
    ...
    gtp_remove_all_nodes(&self.remote_mme_list);
    gtp_node_final();
    ...
    while(yaml_iter_next(&root_iter))
    {
        ...
        if (!strcmp(root_key, "mme"))
        {
            ...
            while(yaml_iter_next(&mme_iter))
            {
                ...
                else if (!strcmp(mme_key, "remote_mme"))
                {
                    yaml_iter_t remote_mme_array, remote_mme_iter;
                    yaml_iter_recurse(&mme_iter, &remote_mme_array);
                    do
                    {
                        gtp_node_t *remote_mme = NULL;
                        c_sockaddr_t *list = NULL;
                        int family = AF_UNSPEC;
                        int i, num = 0;
                        const char *hostname[MAX_NUM_OF_HOSTNAME];
                        ...
                        if (yaml_iter_type(&remote_mme_array) == YAML_MAPPING_NODE)
                        {
                            memcpy(&remote_mme_iter, &remote_mme_array,
                                    sizeof(yaml_iter_t));
                        }
                        else if (yaml_iter_type(&remote_mme_array) ==
                            YAML_SEQUENCE_NODE)
                        {
                            if (!yaml_iter_next(&remote_mme_array))
                                break;
                            yaml_iter_recurse(&remote_mme_array, &remote_mme_iter);
                        }
                        else if (yaml_iter_type(&remote_mme_array) ==
                                YAML_SCALAR_NODE)
                        {
                        ...
                        while(yaml_iter_next(&remote_mme_iter))
                        {
                            const char *remote_mme_key =
                                yaml_iter_key(&remote_mme_iter);
                            d_assert(remote_mme_key,
                                    return CORE_ERROR,);
                            if (!strcmp(remote_mme_key, "family"))
                            {
                                const char *v = yaml_iter_value(&remote_mme_iter);
                                if (v) family = atoi(v);
                                if (family != AF_UNSPEC &&
                                    family != AF_INET && family != AF_INET6)
                                {
                            ...
                            else if (!strcmp(remote_mme_key, "addr") ||
                                    !strcmp(remote_mme_key, "name"))
                            {
                                yaml_iter_t hostname_iter;
                                yaml_iter_recurse(&remote_mme_iter, &hostname_iter);
                                d_assert(yaml_iter_type(&hostname_iter) !=
                                    YAML_MAPPING_NODE, return CORE_ERROR,);
                            ...
                            else if (!strcmp(remote_mme_key, "port"))
                            {
                                const char *v = yaml_iter_value(&remote_mme_iter);
                                if (v) port = atoi(v);
                            }
                            else
                                d_warn("unknown key `%s`", remote_mme_key);
                        }
                        ...
                        rv = gtp_add_node(&self.remote_mme_list, &remote_mme, list,
                                context_self()->parameter.no_ipv4,
                                context_self()->parameter.no_ipv6,
                    ...
                    } while(yaml_iter_type(&remote_mme_array) == YAML_SEQUENCE_NODE);
                }
                else
...
status_t mme_context_setup_trace_module()
{
    ...
    if (gtpv2)
    {
        ...
        extern int _mme_s10_inf;
        d_trace_level(&_mme_s10_inf, gtpv2);
    }
...
void mme_ue_print(mme_ue_t *mme_ue)
{
    int i,n;
    enb_ue_t *enb_ue = NULL;
    char tmp_buf[100];
    ...
    d_assert(mme_ue, return , "mme_ue is NULL");

    enb_ue = mme_ue->enb_ue;

    d_trace(9,"******************************\n");
    d_trace(9,"index: %d\n",mme_ue->index);
    d_trace(9,"IMSI_BCD: %s\n",mme_ue->imsi_bcd);
    d_trace(9,"IMSI(len = %d): \n",mme_ue->imsi_len);
    for (i = 0,n = 0; i < mme_ue->imsi_len; i++)
    {
        n += sprintf(tmp_buf+n,"%02x ", mme_ue->imsi[i]);
    }
    d_trace(9,"%s\n",tmp_buf);

    d_trace(9,"GUTI: plmn(%u %u %u , %u %u %u) GID: %u Code: %u m_tmsi:0x%0x\n",
            mme_ue->guti.plmn_id.mcc1,
            mme_ue->guti.plmn_id.mcc2,
            mme_ue->guti.plmn_id.mcc3,
            mme_ue->guti.plmn_id.mnc1,
            mme_ue->guti.plmn_id.mnc2,
            mme_ue->guti.plmn_id.mnc3,
            mme_ue->guti.mme_gid,
            mme_ue->guti.mme_code,
            mme_ue->guti.m_tmsi);
    if (enb_ue)
    {
        d_trace(9,"MME UE S1AP ID : %d\n",enb_ue->mme_ue_s1ap_id);
        d_trace(9,"ENB UE S1AP ID : %d\n",enb_ue->enb_ue_s1ap_id);
    }

    d_trace(9,"S11 TEID: %d\n",mme_ue->mme_s11_teid);
    d_trace(9,"SGW TEID: %d\n",mme_ue->sgw_s11_teid);
    d_trace(9,"SCTP ostream_id : %d\n",mme_ue->ostream_id);

    d_trace(9,"TAI: plmn(%u %u %u , %u %u %u) tac: %d\n",
                mme_ue->tai.plmn_id.mcc1,
                mme_ue->tai.plmn_id.mcc2,
                mme_ue->tai.plmn_id.mcc3,
                mme_ue->tai.plmn_id.mnc1,
                mme_ue->tai.plmn_id.mnc2,
                mme_ue->tai.plmn_id.mnc3,
                mme_ue->tai.tac);
    d_trace(9,"ECGI: plmn(%u %u %u , %u %u %u) cell_id : %d\n",
                mme_ue->e_cgi.plmn_id.mcc1,
                mme_ue->e_cgi.plmn_id.mcc2,
                mme_ue->e_cgi.plmn_id.mcc3,
                mme_ue->e_cgi.plmn_id.mnc1,
                mme_ue->e_cgi.plmn_id.mnc2,
                mme_ue->e_cgi.plmn_id.mnc3,
                mme_ue->e_cgi.cell_id);
    d_trace(9,"Visited PLMN: (%u %u %u , %u %u %u)\n",
                mme_ue->visited_plmn_id.mcc1,
                mme_ue->visited_plmn_id.mcc2,
                mme_ue->visited_plmn_id.mcc3,
                mme_ue->visited_plmn_id.mnc1,
                mme_ue->visited_plmn_id.mnc2,
                mme_ue->visited_plmn_id.mnc3);
    d_trace(9,"UE Network Capa: len = %d\n",mme_ue->ue_network_capability.length);
    p = (unsigned char *)&mme_ue->ue_network_capability;
    /* skip length byte and print data */
    p++;

    for (i=0,n=0;i<mme_ue->ue_network_capability.length;i++)
    {
        n+= sprintf(tmp_buf+n,"%02x ",*(p + i));
    }
    d_trace(9,"%s\n",tmp_buf);

    d_trace(9,"MS Network Capa: len = %d\n",mme_ue->ms_network_capability.length);
    p = (unsigned char *)&mme_ue->ms_network_capability;
    /* skip length byte and print data */
    p++;
    for (i=0,n=0;i<mme_ue->ms_network_capability.length;i++)
    {
        n+= sprintf(tmp_buf+n,"%02x ",*(p + i));
    }
    d_trace(9,"%s\n",tmp_buf);
    ...
    {
        d_trace(9,"XRES(Len = %d): \n",mme_ue->xres_len);
        for (i=0,n=0;i<mme_ue->xres_len;i++)
        {
            n+= sprintf(tmp_buf+n,"%02x ",mme_ue->xres[i]);
        }
        d_trace(9,"%s\n",tmp_buf);

        d_trace(9,"KASME: ");
        for (i=0,n=0;i<SHA256_DIGEST_SIZE;i++)
        {
            n+= sprintf(tmp_buf+n,"%02x ",mme_ue->kasme[i]);
        }
        d_trace(9,"%s\n",tmp_buf);

        d_trace(9,"RAND: ");
        for (i=0,n=0;i<RAND_LEN;i++)
        {
            n+= sprintf(tmp_buf+n,"%02x ",mme_ue->rand[i]);
        }
        d_trace(9,"%s\n",tmp_buf);

        d_trace(9,"KNAS_INT: ");
        for (i=0,n=0;i<SHA256_DIGEST_SIZE/2;i++)
        {
            n+= sprintf(tmp_buf+n,"%02x ",mme_ue->knas_int[i]);
        }
        d_trace(9,"%s\n",tmp_buf);
        ...
        for (i=0,n=0;i<SHA256_DIGEST_SIZE/2;i++)
        {
            n+= sprintf(tmp_buf+n,"%02x ",mme_ue->knas_enc[i]);
        }
        d_trace(9,"%s\n",tmp_buf);
        ...
        for (i=0,n=0;i<SHA256_DIGEST_SIZE;i++)
        {
            n+= sprintf(tmp_buf+n,"%02x ",mme_ue->kenb[i]);
        }
        d_trace(9,"%s\n",tmp_buf);
#endif
        ...
        for (i=0,n=0;i<SHA256_DIGEST_SIZE;i++)
        {
            n+= sprintf(tmp_buf+n,"%02x ",mme_ue->nh[i]);
        }
        d_trace(9,"%s\n",tmp_buf);

        d_trace(9,"Enc Algo: %d\n",mme_ue->selected_enc_algorithm);
        d_trace(9,"Int Algo: %d\n",mme_ue->selected_int_algorithm);

        d_trace(9,"Uplink AMBR(Kbps): %d\n",mme_ue->subscription_data.ambr.uplink/1000);
        d_trace(9,"Downlihnk AMBR(Kbps): %d\n",mme_ue->subscription_data.ambr.downlink/1000);

    }
    else
    {
        d_trace(9,"Security Context is not available\n");
    }
...
mme_remote_context_t *mme_remote_context_find_by_ip(ip_t *addr)
{
    mme_remote_context_t *iter = NULL;

    d_trace(9,"Try to find mme_remote_context by %u.%u.%u.%u",
            addr->addr6[0],
            addr->addr6[1],
            addr->addr6[2],
            addr->addr6[3]);

    for (iter = list_first(&self.mme_remote_context_list);
            iter; iter = list_next(iter))
    {
        if (!memcmp(&(iter->addr), addr, sizeof(ip_t)))
        {
            d_trace2(9," => Found\n");
            return iter;
        }
    ...
    d_trace2(9," => Not Found\n");

    return NULL;
}

mme_remote_context_t *mme_remote_context_create(ip_t *addr)
{
    mme_remote_context_t *mme = NULL;

    index_alloc(&mme_remote_context_pool, &mme);
    d_assert(mme, return NULL, "Null param");

    mme->addr = *addr;
    list_init(&mme->ue_list);

    list_append(&self.mme_remote_context_list, mme);

    d_trace(9,"Create mme_remote_context(IPv4:  %u.%u.%u.%u)\n",
            addr->addr6[0],
            addr->addr6[1],
            addr->addr6[2],
            addr->addr6[3]);
...
mme_remote_ue_t *mme_remote_ue_find_by_imsi(mme_remote_context_t * mme, 
                                            c_uint8_t *imsi, int len)
{
    mme_remote_ue_t *iter = NULL;

    d_trace(9,"Try to find mme_remote_ue by imsi -> ");
    for (iter = list_first(&mme->ue_list);
            iter; iter = list_next(iter))
    {
        if (!memcmp(iter->imsi, imsi, len))
        {
            d_trace2(9,"Found\n");
            return iter;
        }
...
mme_remote_ue_t *mme_remote_ue_create(mme_remote_context_t *mme,
                        c_uint8_t *imsi, int len)
{
    c_int8_t        imsi_bcd[MAX_IMSI_BCD_LEN+1];
    mme_remote_ue_t *ue = NULL;

    index_alloc(&mme_remote_ue_pool, &ue);
    d_assert(ue, return NULL, "Null param");

    ue->imsi_len = len;
    memcpy(ue->imsi, imsi, len);

    list_append(&mme->ue_list, ue);

    core_buffer_to_bcd(imsi, len, imsi_bcd);
    d_trace(9,"Create mme_remote_ue -> imsi : %s\n",imsi_bcd);

    return ue;
}

mme_ue_t *mme_ue_add_by_remote_ue(mme_remote_ue_t *mme_remote_ue)
{
    mme_ue_t *mme_ue = NULL;
    mme_sess_t *sess = NULL;
    mme_bearer_t *bearer = NULL;
    ...
    /* FIXME: ostream_id included ? */
#if 0
    mme_ue->ostream_id = mme_remote_ue->ostream_id;
#else
    mme_ue->ostream_id = mme_remote_ue->ostream_id;
#endif
...
#else
    /* Copy GUTI */
    /* FIXME: the previous GUTI is meaningful in the current MME ? */
    memcpy(&mme_ue->guti, &mme_remote_ue->guti, sizeof(guti_t));
#endif

    /*  Copy IMSI */
    mme_ue->imsi_len = mme_remote_ue->imsi_len;
    memcpy(mme_ue->imsi, mme_remote_ue->imsi, mme_remote_ue->imsi_len);
    snprintf(mme_ue->imsi_bcd,sizeof(mme_ue->imsi_bcd),"%s",mme_remote_ue->imsi_bcd);

    /* TAI, ECGI, visited plmn_id */
    memcpy(&mme_ue->tai, &mme_remote_ue->tai, sizeof(tai_t));
    memcpy(&mme_ue->e_cgi, &mme_remote_ue->e_cgi, sizeof(e_cgi_t));
    memcpy(&mme_ue->visited_plmn_id, &mme_remote_ue->visited_plmn_id, sizeof(plmn_id_t));


    /* Security context */
    memcpy(&mme_ue->ue_network_capability, 
            &mme_remote_ue->ue_network_capability, sizeof(nas_ue_network_capability_t));
    memcpy(&mme_ue->ms_network_capability, 
            &mme_remote_ue->ms_network_capability, sizeof(nas_ms_network_capability_t));

    mme_ue->xres_len = mme_remote_ue->xres_len;
    memcpy(mme_ue->xres, mme_remote_ue->xres, mme_ue->xres_len);

    memcpy(mme_ue->kasme, mme_remote_ue->kasme, sizeof(mme_ue->kasme));
    memcpy(mme_ue->rand, mme_remote_ue->rand, sizeof(mme_ue->rand));
    memcpy(mme_ue->autn, mme_remote_ue->autn, sizeof(mme_ue->rand));

    memcpy(mme_ue->knas_int, mme_remote_ue->knas_int, sizeof(mme_ue->knas_int));
    memcpy(mme_ue->knas_enc, mme_remote_ue->knas_enc, sizeof(mme_ue->knas_enc));

    mme_ue->ul_count.i32 = mme_remote_ue->ul_count;
    mme_ue->dl_count = mme_remote_ue->dl_count;

    /* FIXME: kenb */

    /* NHCC, NH */
    mme_ue->nhcc = mme_remote_ue->nhcc;
    memcpy(mme_ue->nh, mme_remote_ue->nh, sizeof(mme_ue->nh));

    /* security algorithms */
    mme_ue->selected_enc_algorithm = mme_remote_ue->selected_enc_algorithm;
    mme_ue->selected_int_algorithm = mme_remote_ue->selected_int_algorithm;

    mme_ue->subscription_data.ambr.downlink = mme_remote_ue->ambr_downlink*1000;
    mme_ue->subscription_data.ambr.uplink = mme_remote_ue->ambr_uplink*1000;

    //for(int i = 0; i<mme_remote_ue->num_of_session; i++)
    /* Create session */
    if (mme_remote_ue->num_of_session)
    {
        s6a_subscription_data_t *subscription_data = NULL;
        ...
        //subscription_data->num_of_pdn = mme_remote_ue->num_of_session;
        subscription_data->num_of_pdn = 1;
        strcpy(subscription_data->pdn[0].apn,
                mme_remote_ue->session[0].pdn.apn);
    }
    /* Network capability */
    memcpy(&mme_ue->ue_network_capability, 
            &mme_remote_ue->ue_network_capability, sizeof(mme_remote_ue->ue_network_capability));

    memcpy(&mme_ue->ms_network_capability, 
            &mme_remote_ue->ms_network_capability, sizeof(mme_remote_ue->ms_network_capability));
    ...
    event_set_param1(&e, (c_uintptr_t)mme_ue->index);
    /* The initial state will be registered state */
    fsm_create(&mme_ue->sm, emm_state_registered, emm_state_final);
    fsm_init(&mme_ue->sm, &e);

    return mme_ue;
}

void mme_remote_ue_print(mme_remote_ue_t *mme_remote_ue)
{
    char tmp_buf[100];
    int i,n;
    unsigned char *p = NULL;

    d_assert(mme_remote_ue, return, "mme_remote_ue is null");

    d_trace(9,"******************************\n");
    d_trace(9,"index: %d\n",mme_remote_ue->index);
    d_trace(9,"IMSI_BCD: %s\n",mme_remote_ue->imsi_bcd);
    d_trace(9,"IMSI(len = %d): \n",mme_remote_ue->imsi_len);
    for (i = 0,n = 0; i < mme_remote_ue->imsi_len; i++)
    {
        n += sprintf(tmp_buf+n,"%02x ", mme_remote_ue->imsi[i]);
    }
    d_trace(9,"%s\n",tmp_buf);

    d_trace(9,"GUTI: plmn(%u %u %u , %u %u %u) GID: %u Code: %u m_tmsi:0x%0x\n",
            mme_remote_ue->guti.plmn_id.mcc1,
            mme_remote_ue->guti.plmn_id.mcc2,
            mme_remote_ue->guti.plmn_id.mcc3,
            mme_remote_ue->guti.plmn_id.mnc1,
            mme_remote_ue->guti.plmn_id.mnc2,
            mme_remote_ue->guti.plmn_id.mnc3,
            mme_remote_ue->guti.mme_gid,
            mme_remote_ue->guti.mme_code,
            mme_remote_ue->guti.m_tmsi);


    d_trace(9,"MME UE S1AP ID : %d\n",mme_remote_ue->mme_ue_s1ap_id);
    d_trace(9,"ENB UE S1AP ID : %d\n",mme_remote_ue->enb_ue_s1ap_id);

    d_trace(9,"UE Network Capa: len = %d\n",mme_remote_ue->ue_network_capability.length);
    p = (unsigned char *)&mme_remote_ue->ue_network_capability;
    /* skip length byte and print data */
    p++;
    for (i=0,n=0;i<mme_remote_ue->ue_network_capability.length;i++)
    {
        n+= sprintf(tmp_buf+n,"%02x ",*(p + i));
    }
    d_trace(9,"%s\n",tmp_buf);

    d_trace(9,"MS Network Capa: len = %d\n",mme_remote_ue->ms_network_capability.length);
    p = (unsigned char *)&mme_remote_ue->ms_network_capability;
    /* skip length byte and print data */
    p++;
    for (i=0,n=0;i<mme_remote_ue->ms_network_capability.length;i++)
    {
        n+= sprintf(tmp_buf+n,"%02x ",*(p + i));
    }
    d_trace(9,"%s\n",tmp_buf);

    d_trace(9,"XRES(Len = %d): \n",mme_remote_ue->xres_len);
    for (i=0,n=0;i<mme_remote_ue->xres_len;i++)
    {
        n+= sprintf(tmp_buf+n,"%02x ",mme_remote_ue->xres[i]);
    }
    d_trace(9,"%s\n",tmp_buf);
    ...
    for (i=0,n=0;i<SHA256_DIGEST_SIZE;i++)
    {
        n+= sprintf(tmp_buf+n,"%02x ",mme_remote_ue->kasme[i]);
    }
    d_trace(9,"%s\n",tmp_buf);
    ...
    for (i=0,n=0;i<RAND_LEN;i++)
    {
        n+= sprintf(tmp_buf+n,"%02x ",mme_remote_ue->rand[i]);
    }
    d_trace(9,"%s\n",tmp_buf);
    ...
    for (i=0,n=0;i<SHA256_DIGEST_SIZE/2;i++)
    {
        n+= sprintf(tmp_buf+n,"%02x ",mme_remote_ue->knas_int[i]);
    }
    d_trace(9,"%s\n",tmp_buf);
    ...
    for (i=0,n=0;i<SHA256_DIGEST_SIZE/2;i++)
    {
        n+= sprintf(tmp_buf+n,"%02x ",mme_remote_ue->knas_enc[i]);
    }
    d_trace(9,"%s\n",tmp_buf);

    d_trace(9,"DL CNT: %d\n",mme_remote_ue->dl_count);
    d_trace(9,"UL CNT: %d\n",mme_remote_ue->ul_count);
#if 0
    d_trace(9,"KENB : ");
    for (i=0,n=0;i<SHA256_DIGEST_SIZE;i++)
    {
        n+= sprintf(tmp_buf+n,"%02x ",mme_remote_ue->kenb[i]);
    }
    d_trace(9,"%s\n",tmp_buf);
#endif
    ...
    for (i=0,n=0;i<SHA256_DIGEST_SIZE;i++)
    {
        n+= sprintf(tmp_buf+n,"%02x ",mme_remote_ue->nh[i]);
    }
    d_trace(9,"%s\n",tmp_buf);
    d_trace(9,"Enc Algo: %d\n",mme_remote_ue->selected_enc_algorithm);
    d_trace(9,"Int Algo: %d\n",mme_remote_ue->selected_int_algorithm);
    d_trace(9,"Uplink AMBR(Kbps): %d\n",mme_remote_ue->ambr_uplink);
    d_trace(9,"Downlihnk AMBR(Kbps): %d\n",mme_remote_ue->ambr_downlink);
