...
/* 8.38 MM Context(EPS Security Context) */
typedef struct _gtp_mm_context_eps_t {
ED4(c_uint8_t       security_mode:3;,
    c_uint8_t       nhi:1;,
    c_uint8_t       drxi:1;,
    c_uint8_t       ksi_asme:3;)
ED4(c_uint8_t       num_quintuplets:3;,
    c_uint8_t       num_quardruplet:3;,
    c_uint8_t       uambri:1;,
    c_uint8_t       osci:1;)
ED3(c_uint8_t       sambri:1;,
    c_uint8_t       nas_int_algo:3;,
    c_uint8_t       nas_cipher_algo:4;)
    c_uint8_t       nas_dl_count[3];
    c_uint8_t       nas_ul_count[3];
    c_uint8_t       kasme[32];

    /* FIXME: Include One Authentication Quadruplet*/
#define MAX_AUTH_QUARDRUPLET 1
    struct {
        ...
        c_uint8_t       kasme[32];
    } auth_quadruplet[MAX_AUTH_QUARDRUPLET];
    /* FIXME: Skip Authentication Quintuplet */

    /* FIXME: No DRX */
    /* c_uint16_t      drx_parameter; */
    c_uint8_t       nh[32];
    union {
        struct {
    ...
    /* Include Subscribed UE AMBR :  SAMBRI = 1 */
    c_uint32_t      uplink_subscribed_ue_ambr;
    c_uint32_t      downlink_subscribed_ue_ambr;

    /* FIXME: Exclude Used AMBR */
    /*
    c_uint32_t      uplink_used_ue_ambr;
    c_uint32_t      downlink_used_ue_ambr;
    */
...
#else
    struct {
        c_uint8_t  length; /* length should be 7 */
        c_uint8_t  data[7];
    } ue_network_capability;

    struct {
        c_uint8_t  length; /* length should be 4 */
        c_uint8_t  data[4];
    } ms_network_capability;
#endif

    /* FIXME: Other fields */
} __attribute__ ((packed)) gtp_mm_context_eps_t;

CORE_DECLARE(c_int16_t) gtp_build_mm_context(
            tlv_octet_t *octet, gtp_mm_context_eps_t *mm_context, void *data, 
            int data_len);
CORE_DECLARE(c_int16_t) gtp_parse_mm_context(gtp_mm_context_eps_t *mm_context, 
        tlv_octet_t *octet);
