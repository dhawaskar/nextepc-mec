...
static thread_id oam_thread;
static void *THREAD_FUNC oam_main(thread_id id, void *data);
...
#define USRSCTP_LOCAL_UDP_PORT 9899
    ...
    if (rv != CORE_OK) return rv;
    rv = thread_create(&oam_thread, NULL, oam_main, NULL);
    if (rv != CORE_OK) return rv;
...
static void *THREAD_FUNC sm_main(thread_id id, void *data)
{
    ...
    gtp_xact_init(&mme_self()->tm_service,
            MME_EVT_GTPV2_T3_RESPONSE, MME_EVT_GTPV2_T3_HOLDING);
...
static void *THREAD_FUNC oam_main(thread_id id, void *data)
{
    ...
    rv = mme_oam_start();
    d_assert(rv == CORE_OK, return NULL, "OAM initialization failed");
  
#define OAM_LOOP_TIMEOUT 1000   /* 1000ms */
    while ((!thread_should_stop()))
    {
        core_sleep(OAM_LOOP_TIMEOUT * 1000); 
    }
