...
typedef struct _mme_context_t {
    ...
    list_t          remote_mme_list;       /* List of outbound remote MME 
                                              gtp node , which configured from
                                              the file
                                            */
    list_t          mme_remote_context_list; /* List of mme_remote_context */
...
/** Remote MME */
typedef struct _mme_remote_context_t {
    lnode_t         node;   /* A node of list_t */
    index_t         index;  /* An index of this node */

    ip_t            addr;      /* IP address */
    list_t          ue_list;    /* UE List */

} mme_remote_context_t;


/** Remote UE */
typedef struct _mme_remote_ue_t {
    lnode_t         node;   /* A node of list_t */
    index_t         index;  /* An index of this node */
...
#if 0 /* FIXME: kenb shoudl be shared ? */
    c_uint8_t       kenb[SHA256_DIGEST_SIZE];
#endif
    ...
    c_uint32_t      ambr_uplink; /** Kbps */ 
    c_uint32_t      ambr_downlink; /** Kbps */ 

#define MAX_SESSION_NUMBER  2
#define MAX_BEARER_NUMBER  5

    c_uint8_t num_of_session;
    struct {
        c_uint8_t ebi;
        pdn_t pdn;
        c_uint8_t num_of_bearer;;

        struct {
            c_uint8_t ebi;
        } bearer[MAX_BEARER_NUMBER];
    } session[MAX_SESSION_NUMBER];
...
CORE_DECLARE(void)         mme_ue_print(mme_ue_t *mme_ue);

CORE_DECLARE(mme_remote_context_t *) mme_remote_context_find_by_ip(ip_t *addr);
CORE_DECLARE(mme_remote_context_t *) mme_remote_context_create(ip_t *addr);
CORE_DECLARE(mme_remote_ue_t *) mme_remote_ue_find_by_imsi(
                                    mme_remote_context_t * mme_remote_context, 
                                    c_uint8_t *imsi, int len);

CORE_DECLARE(mme_remote_ue_t *) mme_remote_ue_create(
                                    mme_remote_context_t * mme_remote_context,
                                    c_uint8_t *imsi, int len);
CORE_DECLARE(void)          mme_remote_ue_print(mme_remote_ue_t *mme_remote_ue);

CORE_DECLARE(mme_ue_t *)    mme_ue_add_by_remote_ue(mme_remote_ue_t *mme_remote_ue);
