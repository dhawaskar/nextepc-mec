        ...
        mme_enb_t *enb, s1ap_message_t *message)
{
    ...
    while(sess)
    {
	  ...
	  pdn_t *pdn = sess->pdn;
          ...
          char cmd_buffer[80];
          char ip_buffer[80];
          if (mme_ue->tai.tac == 12346)
	  {
            bearer = mme_default_bearer_in_sess(sess);
            d_assert(bearer, return, "Null param");

            /* add ue */
            if (pdn->paa.pdn_type == GTP_PDN_TYPE_IPV4)
            {
                sa.c_sa_family = AF_INET;
                sa.sin.sin_addr.s_addr = pdn->paa.addr;
                sprintf(cmd_buffer, "post_mec_ue %s %s %u %u\n", 
                        mme_ue->imsi_bcd,
                        core_inet_ntop(&sa, ip_buffer, 80),
                        bearer->enb_s1u_teid, bearer->sgw_s1u_teid);
    		d_trace(3, "++++++++++++++++++++++++++++++++++++++\n");
    		d_trace(3, "MEC ATTACH[%s]\n", cmd_buffer);
                system(cmd_buffer);
            }
            else if (pdn->paa.pdn_type == GTP_PDN_TYPE_IPV6)
            {
                /* TODO */
            }
	  }
...
#if 1	/* BRTODO */
    if ((enb_ue->nas.tai.tac == 12346) && (mme_ue->tai.tac != 12346))
        flag = 1;
    else if ((enb_ue->nas.tai.tac != 12346) && (mme_ue->tai.tac == 12346))
        flag = 2;
#endif
          ...
          if (flag == 1)
	  {
          ...
          else if (flag == 2)
          {
            ...
            if (pdn->paa.pdn_type == GTP_PDN_TYPE_IPV4)
            {
                sa.c_sa_family = AF_INET;
                sa.sin.sin_addr.s_addr = pdn->paa.addr;
                sprintf(cmd_buffer, "delete_mec_ue %s %s\n", 
                        mme_ue->imsi_bcd,
                        core_inet_ntop(&sa, ip_buffer, 80));
    		d_trace(3, "--------------------------------------\n");
    		d_trace(3, "MEC X2HO OUT [%s]\n", cmd_buffer);
                system(cmd_buffer);
            }
            else if (pdn->paa.pdn_type == GTP_PDN_TYPE_IPV6)
            {
...
void s1ap_handle_handover_notification(mme_enb_t *enb, s1ap_message_t *message)
{
    ...
    while(sess)
    {
	  ...
	  pdn_t *pdn = sess->pdn;
          ...
          char cmd_buffer[80];
          char ip_buffer[80];
          if ((target_ue->nas.tai.tac == 12346) && (mme_ue->tai.tac != 12346))
	  {
            /* add ue */
            if (pdn->paa.pdn_type == GTP_PDN_TYPE_IPV4)
            {
          ...
          else if ((target_ue->nas.tai.tac != 12346) && (mme_ue->tai.tac == 12346))
          {
            /* del ue */
            if (pdn->paa.pdn_type == GTP_PDN_TYPE_IPV4)
            {
