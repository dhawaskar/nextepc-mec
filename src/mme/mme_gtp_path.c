...
static int _gtpv2_c_recv_cb(sock_id sock, void *data)
{
    ...
    event_set(&e, MME_EVT_GTPV2_MESSAGE);
...
#if 0 /* BRTODO */
          if ((sess->pdn != NULL) && (sess->pdn->paa.pdn_type == GTP_PDN_TYPE_IPV4) && (mme_ue->tai.tac == 12346))
          //if ((sess->pdn != NULL) && (sess->pdn->paa.pdn_type == GTP_PDN_TYPE_IPV4))
          {
            c_sockaddr_t sa;
            char cmd_buffer[80];
            char ip_buffer[80];
            ...
            sa.sin.sin_addr.s_addr = sess->pdn->paa.addr;
